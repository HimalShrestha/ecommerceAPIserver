...
router.get('/', function(req,res){
  ...
  db.pool.query(`SELECT products.ProductID,products.ProductName,products.ProductPrice,products.ProductCartDesc,
      ...
      INNER JOIN productcategories ON products.ProductCategoryID = productcategories.CategoryID ORDER BY products.ProductID DESC `+limit).then(function(result){
    db.pool.query(`SELECT COUNT(*) FROM products INNER JOIN sellers ON products.ProductSellerID = sellers.SellerID
    INNER JOIN productcategories ON products.ProductCategoryID = productcategories.CategoryID`).then(function(count){
...
router.get('/category', function(req,res){
  ...
  var limit = ' LIMIT ' +size+' OFFSET '+(size*page)
  db.pool.query(`SELECT * FROM productcategories ORDER BY CategoryID DESC `+limit).then(function(result){
    db.pool.query(`SELECT COUNT(*) FROM productcategories`).then(function(count){
          res.status(200).send({data:result[0],count:count[0][0]['COUNT(*)']})
        }).catch(function(err){
          console.log(err)
          res.send(err)
